...
export default function WishlistPage() {
  const { wishlistIds, toggleWishlist, addToCart, navigate } = useStore();
  ...
  return (
    <div className="min-h-screen">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        ...
        {loading ? (
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">{Array.from({ length: 4 }).map((_, i) => <div key={i} className="aspect-[3/4] bg-muted animate-pulse rounded-lg" />)}</div>
        ) : products.length === 0 ? (
          <div className="text-center py-20">
            ...
            <Button onClick={() => navigate('shop')} className="bg-gold text-background hover:bg-gold-dark">Browse Collection</Button>
          </div>
        ) : (
                  ...
                  <div className="product-card group cursor-pointer bg-card rounded-lg overflow-hidden border border-border" onClick={() => navigate('product', product.id)}>
                    <div className="relative aspect-[3/4] img-zoom">
                      ...
                      <div className="absolute bottom-0 left-0 right-0 p-3 bg-gradient-to-t from-black/60 to-transparent opacity-0 group-hover:opacity-100 transition-opacity">
                        <Button onClick={(e) => { e.stopPropagation(); const sizes = parseJsonField<string>(product.sizes); addToCart(product, 1, sizes[0]); }} className="w-full bg-white text-foreground hover:bg-gold hover:text-background text-xs tracking-wider uppercase h-9">
